...
items = os.listdir("/home/eman.ali/Music/data/flowers/train")


# Iterate through each item
for item in items:
    item_path = os.path.join("/home/eman.ali/Music/data/flowers/train", item)
    # Check if it's a directory
    if os.path.isdir(item_path):
        # Check if the folder name matches the old_name
        if item in d.keys():
            # Construct the new path with the new folder name
            new_path = os.path.join("/home/eman.ali/Music/data/flowers/train", d[item])
            # Rename the folder
